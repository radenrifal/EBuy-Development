...
class DDM_Model extends CI_Model
{
    /* --------------------------------------------------------------
    * VARIABLES
    * ------------------------------------------------------------ */
    ...
    * ------------------------------------------------------------ */
    ...
    public function __construct()
    {
        ...
        array_unshift($this->before_create, 'protect_attributes', 'switch_database_write');
        array_unshift($this->before_update, 'protect_attributes', 'switch_database_write');
        array_unshift($this->before_delete, 'switch_database_write');

        array_unshift($this->after_create, 'switch_database_write');
        array_unshift($this->after_update, 'switch_database_write');
        array_unshift($this->after_delete, 'switch_database_write');
    ...
    * ------------------------------------------------------------ */
     ...
     * 
     * WARNING!!! DO NOT overwrite this method unless you understand what you're doing!
     */
    ...
    * ------------------------------------------------------------ */
    ...
    /**
     * Direct ActiveRecord select
     * @author Ahmad
     * @param  string   [$select         = '*']  Select
     * @param  boolean  [$escape         = NULL] Escape
     */
    ...
    * ------------------------------------------------------------ */
    ...
    function dropdown()
    {
        ...
        $result = $this->_database->select(array($key, $value))->get($this->_table)->result();
    ...
    {
        return (int)$this->_database->select('AUTO_INCREMENT')->from('information_schema.TABLES')->
            where('TABLE_NAME', $this->_table)->where('TABLE_SCHEMA', $this->_database->
            database)->get()->row()->AUTO_INCREMENT;
    }
    ...
    * ------------------------------------------------------------ */
    ...
    public function serialize($row)
    {
        foreach ($this->callback_parameters as $column) {
            $row[$column] = maybe_serialize($row[$column]);
        }
    ...
    * ------------------------------------------------------------ */
    ...
    public function switch_database_write($data)
    {
        $this->_database = $this->db;
        return $data;
    }

    public function switch_database_read()
    {
        $this->_database = $this->db;
    }
}
