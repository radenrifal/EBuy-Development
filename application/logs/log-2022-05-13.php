...
ERROR - 2022-05-13 13:58:11 --> Query error: FUNCTION freshids_dbs.GetLineActive does not exist - Invalid query: SELECT SQL_CALC_FOUND_ROWS 
...
ERROR - 2022-05-13 14:00:36 --> Query error: FUNCTION freshids_dbs.GetLineActive does not exist - Invalid query: SELECT SQL_CALC_FOUND_ROWS 
                        IFNULL(COUNT(*), 0) AS total_trx,
                        IFNULL( SUM(G.`omzet`), 0 ) AS total_omzet,
                        IFNULL( SUM(G.`bv`), 0 ) AS total_bv,
                        IFNULL( SUM(G.`amount`), 0 ) AS total_amount,
